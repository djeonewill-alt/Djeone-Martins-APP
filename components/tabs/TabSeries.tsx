    ...
    <div className={`relative overflow-hidden bg-slate-900 ${className}`}>
      {imageUrl ? (
        <img
          src={imageUrl}
          alt={serie.title}
          className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-active:scale-[1.03]"
        />
      ) : (
...
function FeaturedHero({ serie }: { serie: Series }) {
  return (
...
function WideSeriesCard({ serie }: { serie: Series }) {
  return (
    <button
      type="button"
...
function CompactSeriesCard({ serie }: { serie: Series }) {
  return (
    <button
      type="button"
  ...
  return (
    <section className="mb-8">
      ...
      <div className="-mx-5 flex gap-4 overflow-x-auto px-5 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {items.map((serie) =>
          variant === 'wide' ? (
            <WideSeriesCard key={serie.id} serie={serie} />
          ) : (
            <CompactSeriesCard key={serie.id} serie={serie} />
          )
        )}
      </div>
    </section>
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-5 pb-32 pt-20 text-white">
      <div className="mx-auto max-w-2xl">
        ...
        {featuredSeries && <FeaturedHero serie={featuredSeries} />}
